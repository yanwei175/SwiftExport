...
using CurrentEntity = SwiftExport.Core.Entities.Customers;
using CurrentDto = SwiftExport.AppLayer.DTOs.CustomersDto;
...
namespace SwiftExport.UiKongJianFrameWork.AppForms
{
    public partial class FrmCustomersManager : Form
    {
        ...
        public FrmCustomersManager(ICustomersService service,IMapper mapper)
        {
            ...
            Dgv1.EnableDoubleBuffered()
                ...
                .SetNotNullColumnsColumns(nameof(CurrentDto.客户代码), nameof(CurrentDto.单据关联))//设置不能为空列
                .SetUniqueColumns(nameof(CurrentDto.客户代码))//设置唯一列
                .EnableDeleteKey()//设置Dlete清空单元格
                .SetAutoUcaseColumn(nameof(CurrentDto.客户代码), nameof(CurrentDto.单据关联))
        ...
        private async void TsBtn_Add_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    var cell = Dgv1.Rows[0].Cells["客户代码"];
            ...
            Dgv1.CurrentCell = Dgv1.Rows[0].Cells["客户代码"];
