...
using CurrentEntity = SwiftExport.Core.Entities.Suppliers;
using CurrentDto = SwiftExport.AppLayer.DTOs.SuppliersDto;
...
namespace SwiftExport.UiKongJianFrameWork.AppForms
{
    public partial class FrmSuppliersManager : Form
    {
        ...
        public FrmSuppliersManager(ISuppliersService service,IMapper mapper)
        {
            ...
            Dgv1.EnableDoubleBuffered()
                ...
                .SetNotNullColumnsColumns(nameof(CurrentDto.供应商代码), nameof(CurrentDto.单据关联), nameof(CurrentDto.供应商名称))//设置不能为空列
                .SetUniqueColumns(nameof(CurrentDto.供应商代码), nameof(CurrentDto.供应商名称))//设置唯一列
                .EnableDeleteKey()//设置Dlete清空单元格
                .SetAutoUcaseColumn(nameof(CurrentDto.供应商代码), nameof(CurrentDto.单据关联))
        ...
        private async void TsBtn_Add_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    var cell = Dgv1.Rows[0].Cells["供应商代码"];
            ...
            Dgv1.CurrentCell = Dgv1.Rows[0].Cells["供应商代码"];
        ...
        private async Task UpdateAsync(CurrentDto sp)
        {
            try
            {
                ...
                if (rst.Success)
                {
                    BdSource1.SilentUpdate(() => { sp.视图状态 = RowState.正常; });
